...
public class LexVin extends Lex {
	...
	public LexVin(InputStream flot) {
		...
		super(flot);
		System.out.println("Trinome MESSAGER_Clément_LEPORT_ETEL"); // TODO : maj avec vos noms
		System.out.println("--------------");
	...
	private int lireIdent() {
	    ...
	    if (ident.equals("BOURGOGNE")) {
	        return BOURGOGNE;
	    } else if (ident.equals("BEAUJOLAIS")) {
	        return BEAUJOLAIS;
	    } else {
	        if (!tabIdent.contains(ident)) {
	            tabIdent.add(ident);
	        }
	        numIdCourant = tabIdent.indexOf(ident);
	        return IDENT;
	    }
	}
	...
	public int lireSymb() {
		...
		case '/': 
			return BARRE;
		default: {
			System.out.println("LesExp : caractere incorrecte : " + carLu);
			lireCarLu();
			return AUTRES;
		}
		}
